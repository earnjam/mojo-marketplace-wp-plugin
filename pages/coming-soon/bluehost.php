...
<body>
			...
			<?php if ( class_exists( 'Jetpack' ) && Jetpack::is_module_active( 'subscriptions' ) ) { ?>
				...
				<div class="bh_subscription_widget">
					<h2 class="widgettitle">Coming Soon!</h2>
						...
						<div id="error">There was an error with your submission and you were not subscribed. Please try again with a valid email address.</div>
	...
	<footer>
			<p class='text-center'>A <a href='https://bluehost.com/wordpress' class='bluehost' rel='nofollow'>Bluehost</a> powered website. Is this your website? Log in to <a href='<?php echo site_url( 'wp-login.php' ); ?>'>WordPress</a> or <a href='https://my.bluehost.com/' class='bluehost' rel='nofollow'>Bluehost</a></p>
	</footer>
<script>
		...
		$( '#subscribe-submit input' ).click(function(e){
			...
			$.ajax({
				...
				error: function( result ){
					...
					//var resuobj = jQuery.parseJSON( resu );
					//if ( resuobj.status == 200 ){
						console.log( 'There was an error!! ' + resu );
						$( '#error' ).show();
					//}
				},
			});
